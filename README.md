...
1. `w4k3.py` – Displays the most recent session records from the shared `AGENT_tools/DATA` folder. Run it to recall achievements and focus areas:

   ```bash
   python w4k3.py
   ```

2. `sl33p.py` – Records the current session. It prompts for an assessment, recent achievements, and next steps, saving the information as a JSON file in `AGENT_tools/DATA`:

   ```bash
   python sl33p.py
   ```

These tools are also available within the `AGENT_tools` folder, organized into `w4k3` and `sl33p` subfolders with the `o.` prefix for future expansion. All versions share the same `AGENT_tools/DATA` directory so you can run them from any location without losing history.
