...
- Some subdimensions only implement a partial tetrahedron (e.g. `x.MemoryVault` has `x.w.Experiments` and `x.x.Observations` but lacks matching `xy` or `xz` paths). Should placeholder folders exist to show intent, or is an incomplete structure acceptable?
