...
2. **Copy** – Behavioral patterns and phenomenology
   - Group all `*.PHENO.md` files under a new folder `x.MemoryVault/` to emphasize their exploratory nature. Cross‑link specific F33ling states in `AGENT.md` to the corresponding phenomenology notes.
3. **Control** – Workflow utilities and navigation guides
   - Retain the operational tools under `y.Utilities/yz.AgentOps/`, along with `workflow.sh` and installation hooks.
...
Adopting these categories will reduce root clutter and make it clearer which tetrahedral vertex each document belongs to. Centralizing phenomenology under `x.MemoryVault/` keeps exploratory narratives distinct from workflow directives, while consolidating duplicated instructions improves newcomer orientation.
